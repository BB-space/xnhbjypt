...
public class MoneyUtils {
	...
	public static String createFuWuHaoSign(Map<String, Object> map,String pay_api_key) {
		SortedMap<String, String> packageParams = new TreeMap<String, String>();
		for (Map.Entry<String, Object> m : map.entrySet()) {
			packageParams.put(m.getKey(), m.getValue().toString());
		}
	...
	public static String getOrderNo() {
		String order = ConfigUtil.COMPAMY_MCH_ID
	...
	public static String getFuWuHaoOrderNo(String mch_id) {
		String order = mch_id
				+ new SimpleDateFormat("yyyyMMdd").format(new Date());
		Random r = new Random();
		for (int i = 0; i < 10; i++) {
	...
	final static String KEYSTORE_FILE = "E:/cert/sykjcompamy/apiclient_cert.p12";
	final static String KEYSTORE_PASSWORD = "1314812001";
	...
	public static String doSendMoney(String url, String data) throws Exception {
		...
		InputStream instream = MoneyUtils.class.getClassLoader().getResourceAsStream("apiclient_cert.p12");
        try {
	 ...
	 * 发送到服务号
	 */
	public static String doSendMoneyFuWuHao(String url,String mch_id, String data) throws Exception {
		KeyStore keyStore  = KeyStore.getInstance("PKCS12");
		//FileInputStream instream = new FileInputStream(new File(KEYSTORE_FILE));//P12文件目录
		InputStream instream = MoneyUtils.class.getClassLoader().getResourceAsStream(ConfigUtil.KEYSTORE_FILE);
        try { 
