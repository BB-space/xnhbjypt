...
$(function() {
	...
	treegrid = $('#treegrid').treegrid({
		url : '${pageContext.request.contextPath}/widgetSetting/treegrid.do',
		toolbar : [ {
			text : '展开',
			iconCls : 'icon-redo',
			handler : function() {
...
function append() {
	window.location.href="${pageContext.request.contextPath}/widgetSetting/addSlide.do";
}
function editRow(id){
	window.location.href="${pageContext.request.contextPath}/widgetSetting/editSlide.do?id="+id;
}
function delRow(id) {
	if (confirm('你确定要删除该记录?如果该记录有子类会一同删除')) {
		$.ajax({
			type : 'POST',
			url : '${pageContext.request.contextPath}/widgetSetting/del.do',
			data : "id=" + id,
