...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		treegrid = $('#treegrid').treegrid({
			url : '${pageContext.request.contextPath}/business/treegrid.do',
			toolbar : [ {
				text : '展开',
				iconCls : 'icon-redo',
				handler : function() {
			...
			columns : [ [ {
				...
				editor : {
					...
					options : {
						url : '${pageContext.request.contextPath}/resources/tree.do',
						animate : false,
						lines : !sy.isLessThanIe8(),
						onLoadSuccess : function(node, data) {
							var t = $(this);
				...
				width : 200,
				formatter : function(value, rowData, rowIndex) {
					return rowData.menuText;
				},
				editor : {
					type : 'combocheckboxtree',
					options : {
						url : '${pageContext.request.contextPath}/menu/tree.do',
						animate : false,
						lines : !sy.isLessThanIe8(),
						onLoadSuccess : function(node, data) {
							var t = $(this);
			...
			onAfterEdit : function(row, changes) {
				...
				if (row.parentId != row.id) {
					$.ajax({
						url : '${pageContext.request.contextPath}/business/edit.do',
						data : row,
						cache : false,
						dataType : "json",
						success : function(r) {
	...
	function append() {
		if (editRow) {
			...
			$.ajax({
				url : '${pageContext.request.contextPath}/business/add.do',
				data : data[0],
				cache : false,
				dataType : "json",
				success : function(r) {
	...
	function remove() {
		...
		if (node) {
			$.messager.confirm('询问', '您确定要删除【' + node.text + '】角色？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/business/del.do',
						data : {
							id : node.id
						},
						cache : false,
