...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		treegrid = $('#treegrid').treegrid({
			url : '${pageContext.request.contextPath}/resources/treegrid.do',
			toolbar : [ {
				text : '展开',
				iconCls : 'icon-redo',
				handler : function() {
			...
			onAfterEdit : function(row, changes) {
				if (row.parentId != row.id) {
					$.ajax({
						url : '${pageContext.request.contextPath}/resources/edit.do',
						data : row,
						cache : false,
						dataType : "json",
						success : function(r) {
	...
	function append() {
		if (editRow) {
			...
			$.ajax({
				url : '${pageContext.request.contextPath}/resources/add.do',
				data : data[0],
				cache : false,
				dataType : "json",
				success : function(r) {
	...
	function remove() {
		...
		if (node) {
			$.messager.confirm('询问', '您确定要删除【' + node.text + '】资源？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/resources/del.do',
						data : {
							id : node.id
						},
						cache : false,
