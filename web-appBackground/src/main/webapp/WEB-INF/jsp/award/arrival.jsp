...
<script type="text/javascript">
	...
	$(function() {
		...
		memberDialog = $('#memberDialog').show().dialog({
			modal : true,
			height : 450,
			title : '请选择会员信息：选中一条数据，再点【确定】',
			top : '0px',
			buttons : [ { 
				text : '关闭',
				handler : function() {
...
// 		giftDatagrid.datagrid('loadData',{total:0, rows:[]});
	}
	function clearData(){
		$('#toolbarCreateGift input').val('');
	}
...
// 		alert("mobile:"+mobile);
	...
	function selectMember(){
		...
		if(mobile != "" && number >0){
			...
			$.post("arrival.do?memberDatagrid",{
				"mobile":mobile
				},function(data){
				data = $.parseJSON(data);
				memberDatagrid = $("#memberDatagrid").datagrid({
					url : '',
					title : '',
					iconCls : 'icon-save',
					pagination : false,
				...
				$("#memberDatagrid").datagrid('loadData',data);
				$.messager.progress('close');
			});
		}else{
			$.messager.show({
				msg : '请输入用户及补推个数到【补推红包】栏！',
				title : '提示'
			});
		}
	}
...
<body class="easyui-layout" fit="true">
	<div region="center" border="true">
		<div id="toolbarAll" class="datagrid-toolbar" style="height: auto;">
			...
			<fieldset id="toolbarCreateGift" style="border:1px groove #f0f0f0;background-color:#ffffff">
				<legend>补推红包</legend>
				<table class="tableForm">
					<tr>
						<th>会员号码</th>
						<td><input id="mobile3"></td>
						<th>个数</th>
						<td><input id="number"></td>
						<td>
							<a class="easyui-linkbutton" iconCls="icon-search"
							plain="true" onclick="selectMember();" href="javascript:void(0);">推送红包</a>
							<a class="easyui-linkbutton" iconCls="icon-search"
							plain="true" onclick="clearData();" href="javascript:void(0);">清空</a>
						</td>
					</tr>
				</table>
			</fieldset>
	...
	<!-- 弹框:会员-门店信息 -->
	<div id="memberDialog" style="display: none; overflow: hidden;width:50%;height:50%;">
	      <table id="memberDatagrid" height="100%"></table>
    </div>   
</body>
</html>