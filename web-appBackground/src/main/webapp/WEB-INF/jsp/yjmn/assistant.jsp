...
<script type="text/javascript">
	...
	var passwrodInput;
	$(function() {
		assistantForm = $("#assistantForm").form();

		passwordInput = assistantForm.find('[name=name]').validatebox({
			required : true
		});
		assistantDialog = $('#assistantDialog').show().dialog({
			modal : true,
			title : '修改目标',
			top : '0px',
			buttons : [ { 
				text : '确定',
				handler : function() {
					if (assistantForm.find('[name=id]').val() != '') {
						assistantForm.form('submit', {
						...
						assistantForm.form('submit', {
							url : '${pageContext.request.contextPath}/assistant.do?add',
							success : function(data) {
								try {
	...
	function append() {
					$.ajax({
						url : '${pageContext.request.contextPath}/yjmn.do?yjmbSumbit',  
						cache : false,
						dataType : "json",
	...
	function edit() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			assistantForm.find('[name=id]').attr('readonly', 'readonly');
			assistantDialog.dialog('open');
			assistantForm.form('clear');
			assistantForm.form('load', { 
				id : rows[0].id,
...
<body class="easyui-layout" fit="true">
		...
		<div id="toolbar" class="datagrid-toolbar" style="height: auto;">
			<div>
				<a class="easyui-linkbutton" iconCls="icon-add" onclick="append();"
					plain="true" href="javascript:void(0)">提交给店长审核</a> <a
	...
	<div id="assistantDialog" style="display: none; overflow: hidden;">
