...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
				...
				handler : function() {
					if (commonForm.find('[name=id]').val() != '') {
						commonForm.form('submit', {
							url : '${pageContext.request.contextPath}/deliveryRules/edit.do',
						...
						commonForm.form('submit', {
							url : '${pageContext.request.contextPath}/deliveryRules/add.do',
							success : function(data) {
								try {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/deliveryRules/datagrid.do',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
	...
	function edit() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			commonForm.form('load', {
				id : rows[0].id,
				name : rows[0].name,
				deliveryArea : rows[0].deliveryArea,
				expense : rows[0].expense,
				express : rows[0].express
			});
		}
	}
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/deliveryRules/del.do',
						data : {
							ids : ids.join(',')
						},
						cache : false,
		...
		<form id="commonForm" method="post">
			<table class="tableForm">
				...
				<tr>
					<th>运费</th>
					<td>
						<input name="expense" class="easyui-numberbox" type="text" data-options="required:true" />
					</td>
				</tr>
				<tr>
					<th>选择发货地</th>
					<td><input name="deliveryArea" class="easyui-textbox" type="text" data-options="required:true" /></td>
				</tr>
				<tr>
					<th>输入快递全称</th>
					<td><input name="express" class="easyui-textbox" type="text" data-options="required:true"/></td>
				</tr>
			</table>
		</form>
	</div>
