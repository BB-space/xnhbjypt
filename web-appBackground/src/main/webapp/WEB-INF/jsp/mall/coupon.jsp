...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
				...
				handler : function() {
					if (commonForm.find('[name=id]').val() != '') {
						commonForm.form('submit', {
							url : '${pageContext.request.contextPath}/coupon/edit.do',
						...
						commonForm.form('submit', {
							url : '${pageContext.request.contextPath}/coupon/add.do',
							success : function(data) {
								try {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/coupon/datagrid.do',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
			...
			frozenColumns : [ [ {
				...
				formatter:function(value,row,index){
					return "<a href='${pageContext.request.contextPath}/coupon/couponAdd.do?couponId="+row.id+"'>"+value+"</a>";
				}
			} ] ],
			columns :[[{
				field : 'restrictedArea',
				title : '限制区域',
				width : 20,
			},{
				field : 'couponNum',
	...
	function edit() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			commonForm.form('load', {
				...
				validTime : rows[0].validTime,
				invalidTime : rows[0].invalidTime,
			});
		}
	}
	...
	<div id="commonDialog" style="display: none; overflow: hidden;">
		<form id="commonForm" method="post" enctype="multipart/form-data">
			<table class="tableForm">
				...
				<tr>
					<td>开始时间</td>
					<td><input name="validTime" class="easyui-datetimebox" type="text" data-options="required:true"/></td>
				</tr>
				<tr>
					<td>结束时间</td>
					<td><input name="invalidTime" class="easyui-datetimebox" type="text" data-options="required:true"/></td>
				</tr>
			</table>
		</form>
	</div>
