...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
				...
				handler : function() {
					if (commonForm.find('[name=id]').val() != '') {
						commonForm.form('submit', {
							url : '${pageContext.request.contextPath}/goodsDiscount/edit.do',
						...
						commonForm.form('submit', {
							url : '${pageContext.request.contextPath}/goodsDiscount/add.do',
							success : function(data) {
								try {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/goodsDiscount/datagrid.do',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/goodsDiscount/del.do',
						data : {
							ids : ids.join(',')
						},
						cache : false,
		...
		<form id="commonForm" method="post">
			<table class="tableForm">
				...
				<tr>
					<th>编码</th>
					<td>
						<input name="rank" class="easyui-textbox" type="text" data-options="required:true" />
					</td>
				</tr>
				<tr>
					<th>折扣</th>
					<td><input name="discount" class="easyui-textbox" type="text" data-options="required:true"/></td>
				</tr>
			</table>
		</form>
	</div>
