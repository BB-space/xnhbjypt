...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/deliveryManage/datagridOrderItem.do',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
...
<body class="easyui-layout" fit="true">
	<div region="center" border="false" style="overflow: hidden;">
		<div id="toolbar" class="datagrid-toolbar"
			...
			<fieldset>
				<legend>发货单产品信息</legend>
				<table class="tableForm">
					<input type="hidden" value="${deliveryManageM.id}" id="dpfId">
					<tr>
						<td>用户：${deliveryManageM.username}</td>
						<td>发货单号:${deliveryManageM.deliveryNo}</td>
					<tr>
					<tr>
						<td>发货时间:<fmt:formatDate value="${deliveryManageM.deliveryRealTime}" type="both"/></td>
						<td>发货状态:
							<c:if test="${deliveryManageM.deliveryStatus==0}">待总仓拣货</c:if>
							<c:if test="${deliveryManageM.deliveryStatus==1}">待分仓拣货</c:if>
							<c:if test="${deliveryManageM.deliveryStatus==2}">待条码验货</c:if>
							<c:if test="${deliveryManageM.deliveryStatus==3}">待确认发货</c:if>
							<c:if test="${deliveryManageM.deliveryStatus==4}">已发货</c:if>
							<c:if test="${deliveryManageM.deliveryStatus==5}">已收货</c:if>
							<c:if test="${deliveryManageM.deliveryStatus==6}">退回总仓</c:if>
				...
				<a href="javascript:void(0)" class="easyui-linkbutton" onclick="window.location.href='${pageContext.request.contextPath}/deliveryManage/deliveryManage.do'">>>返回</a>
				</div>
			</fieldset>
			<div>
