...
<script type="text/javascript" charset="UTF-8">
	...
	function changePicUrls() {
		...
		var dImg = document.getElementById('pictures');
		if (dFile.files) {
...
<body class="easyui-layout" fit="true">
	<div region="center" border="false" style="overflow: hidden;">
		<div class="head_top">桌面>>商品信息>>添加</div>
		<div class="widget_main">
		
		<form method="post" action="${pageContext.request.contextPath}/goods/add.do"
		    onsubmit="return check()" id="myForm" enctype="multipart/form-data">
			<table class="tableForm" border="0">
				<tr>
					...
					<th>商品类别</th>
					<td>
						<input id="catId" name="catId" style="width:150px;">
					</td>
					<th>商品名称</th>
					<td><input name="name" id="name" class="easyui-textbox" type="text" data-options="required:true" /></td>
					<th>商品编号</th>
					<td>
					<input name="goodsNo" id="goodsNo" type="easyui-validatebox" class="easyui-textbox" data-options="required:true"/>
					</td>
					<th>物品类别</th>
					<td>
						<select name="itemType" id="itemType" class="easyui-combobox" required="true" style="width:150px;">
				...
				<tr>
					<th>价格</th>
					<td><input name="price" id="price" class="easyui-textbox" type="text" data-options="required:true"/></td>
					<th>积分</th>
					<td><input name="integral" id="integral" class="easyui-textbox" type="text" data-options="required:true"/></td>
					<th>库存数量</th>
					<td><input name="kcQuantity" id="kcQuantity" class="easyui-textbox" type="text"/></td>
					<th>上架/下架</th>
					<td>
				...
				<tr>
					<th>规格</th>
					<td><input name="specification" id="specification" class="easyui-textbox" type="text" data-options="required:true"/></td>
					<th>重量</th>
					<td>
					<input name="weight" id="weight" type="easyui-validatebox" class="easyui-textbox" data-options="required:true"/>
					</td>
					<th>商品描述</th>
					<td>
						<input name="description" id="description" class="easyui-textbox" data-options="required:true,multiline:true" style="height:60px;"/>
					</td>
					<th>功效</th>
					<td><input name="efficacy" id="efficacy" class="easyui-textbox" data-options="multiline:true" style="height:60px;"/></td>
					<th>用法</th>
					<td><input name="usage" id="usage" class="easyui-textbox" data-options="multiline:true" style="height:60px;"/></td>
				</tr>
				<tr>
					<th>产地</th>
					...
					<th>标签</th>
					<td><input name="tag" id="tag" class="easyui-textbox" data-options="multiline:true" style="height:60px;"/></td>
					<th>出货规则</th>
					<td>
						<select name="deliveryRulesId" id="deliveryRules" style="width: 100px">
							<c:forEach items="${requestScope.mList }" var="t">
								<option value="${t.id }">${t.express }</option>
							</c:forEach>
						</select>
					</td>
					<th>店员返现</th>
					<td><input name="empRebate" class="easyui-numberbox" min="0" precision="2" max="1"/></td>
					<th>店长返现</th>
					<td><input name="managerRebate" class="easyui-numberbox" min="0" precision="2" max="1"/></td>
				</tr>
				<tr>
					<th>图片</th>
					<td>
						<input id="file" type="file" name="files" onchange="changePicUrls()">
						<img name="pictures" id="pictures" alt="" src=""
						...
						<td>商品LOGO
						<input id="filePic" type="file" name="files" onchange="changePicUrls2()">
