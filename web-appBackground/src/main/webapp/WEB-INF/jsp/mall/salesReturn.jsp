...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
				...
				handler : function() {
					if (commonForm.find('[name=id]').val() != '') {
						...
						commonForm.form('submit', {
							url : '${pageContext.request.contextPath}/salesReturn/edit.do',
						...
						commonForm.form('submit', {
							url : '${pageContext.request.contextPath}/salesReturn/add.do',
							success : function(data) {
								try {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/salesReturn/datagrid.do',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
			...
			frozenColumns : [ [ {
				...
				/* formatter : function(value,row,index){
					return "<a href='/appBackground/salesReturn/salesReturnItem.do?id="+row.id+"&orderId="+row.orderId+"'>"+value+"</a>";
				} */
			} ] ],
			columns :[[{
				field : 'userName',
	...
	function edit() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			commonForm.form('load', {
				...
				isNotSalesReturn : rows[0].isNotSalesReturn
			});
		}
	}
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/salesReturn/del.do',
						data : {
							ids : ids.join(',')
						},
						cache : false,
	...
	function check(){
		var isNotSalesReturn=$("#isNotSalesReturn").val();
		if(isNotSalesReturn==null || isNotSalesReturn==""){
			alert("请选择是否同意退款！");
			return false;
		}else{
			return true;
		...
		<form id="commonForm" method="post" >
			<table class="tableForm">
				...
				<tr>
					<th>是否同意退货</th>
					<td>
						<select name="isNotSalesReturn" id="isNotSalesReturn" style="width:150px;">
