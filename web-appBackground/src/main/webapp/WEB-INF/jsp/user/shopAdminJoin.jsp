...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		userRoleForm = $('#userRoleForm').form({
			url : '${pageContext.request.contextPath}/shopAdminJoin.do?editUsersRole',
				...
				handler : function() {
					if (userForm.find('[name=id]').val() != '') {
						...
						userForm.form('submit', {
							url : '${pageContext.request.contextPath}/shopAdminJoin.do?edit',
							success : function(data) {
								var d = $.parseJSON(data);
								if(d.success==true){
						...
						/* if(!checkData()) return; */
						userForm.form('submit', {
							url : '${pageContext.request.contextPath}/shopAdminJoin.do?add',
							success : function(data) {
								var d = $.parseJSON(data);
								//alert(d.obj);
									if (d.obj=="4") {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/shopAdminJoin.do?datagrid',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
	...
	function edit() {
		...
		if (rows.length != 1 && rows.length != 0) {
			...
			//alert(rows[0].provinceId+'*'+rows[0].cityId+'*'+rows[0].shopId);
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/shopAdminJoin.do?del',
						data : {
							ids : ids.join(',')
						},
						cache : false,
