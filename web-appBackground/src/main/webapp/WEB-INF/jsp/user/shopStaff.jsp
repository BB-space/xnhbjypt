...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
				...
				handler : function() {
					if (userForm.find('[name=id]').val() != '') {
						...
						userForm.form('submit', {
							url : '${pageContext.request.contextPath}/shopStaff.do?edit',
						...
						/* if(!checkData()) return; */
						userForm.form('submit', {
							url : '${pageContext.request.contextPath}/shopStaff.do?add',
							success : function(data) {
								try {
									var d = $.parseJSON(data);
									if (d) {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/shopStaff.do?datagrid',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/shopStaff.do?del',
						data : {
							ids : ids.join(',')
						},
						cache : false,
