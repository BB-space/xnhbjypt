...
<script type="text/javascript" charset="UTF-8">
	 ...
	 */  
	(function(){  
		sessionStorage.removeItem("chooseShop");
	...
	$(function() {
		...
		userDialog = $('#userDialog').show().dialog({
			...
			buttons : [ {
				text : '确定',
				handler : function() {
// 					var shopIds = $('#chooseShop').val();
					var shopIds = sessionStorage.getItem("chooseShop");
// 					var shopIdsArr = shopIds.split(",");
// 					console.log(shopIdsArr);
					if (userForm.find('[name=id]').val() != '') {
						/* if(!checkData())return; */
						userForm.form('submit', {
							url : '${pageContext.request.contextPath}/franchisee.do?edit&chooseShop='+shopIds,
						...
						userForm.form('submit', {
							url : '${pageContext.request.contextPath}/franchisee.do?add&chooseShop='+shopIds,
							success : function(data) {
								try {
									var d = $.parseJSON(data);
									if (d) {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/franchisee.do?datagrid',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
	...
	function showShop(subcenterId) {
		$("#shopIds").attr("disabled",false);
		$('[id=shopIds]').combotree({
			url : '${pageContext.request.contextPath}/shop/tree.do?subcenterId='+subcenterId,
			animate : false,
			lines : !sy.isLessThanIe8(),
			checkbox : true,
			multiple:false,
			...
			onSelect:function(node){
				if(sessionStorage.getItem("shopName") != '' && sessionStorage.getItem("shopName") != null) {
					var sn = sessionStorage.getItem("shopName").split(",");
					var cs = sessionStorage.getItem("chooseShop").split(",");
					var flag = true;
					...
					if(flag) {
						sessionStorage.setItem("shopName",sessionStorage.getItem("shopName")+","+node.text);
						sessionStorage.setItem("chooseShop",sessionStorage.getItem("chooseShop")+","+node.id);
					}
					$("#shopName").val(sessionStorage.getItem("shopName"));
					$("#chooseShop").val(sessionStorage.getItem("chooseShop"));
				}else {
					sessionStorage.setItem("shopName",node.text);
					sessionStorage.setItem("chooseShop",node.id);
					$("#shopName").val(sessionStorage.getItem("shopName"));
					$("#chooseShop").val(sessionStorage.getItem("chooseShop"));
				}
			}
	...
	function clearChoose() {
		...
		$("#shopName").val("");
		$("#chooseShop").val("");
		$("#shopIds").attr("disabled","disabled");
	}
</script>
</head>
	...
	<div id="userDialog" style="display: none; overflow: hidden;width:350px;">
		<form id="userForm" method="post"  enctype="multipart/form-data">
			<table>
				...
				<tr>
					<th>分管分中心</th>
					<td><input id="subcenterId" name="subcenterId" style="width:175px;"></input></td>
				</tr>
				<tr>
					<th>分管门店</th>
					<td><input id="shopIds" name="shopIds" disabled="disabled"></input></td>
				</tr>
				<tr>
					<th>已选门店</th>
					<td><textarea id="shopName" name="shopName" readOnly="true" rows="5" cols="22"></textarea></td>
					<td><input id="chooseShop" name="chooseShop" style="width: 156px;" hidden="true"></input></td>
				</tr>
				<tr>
					<th></th>
					<td><input type="button" value="清空已选门店" onclick="clearChoose()"></td>
				</tr>
			</table>
		</form>
	</div>
