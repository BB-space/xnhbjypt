...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
				...
				handler : function() {
					if (userForm.find('[name=id]').val() != '') {
						...
						userForm.form('submit', {
							url : '${pageContext.request.contextPath}/staff.do?edit',
							success : function(data) {
								var d = $.parseJSON(data);
								if(d.success==true){
						...
						/* if(!checkData()) return; */
						userForm.form('submit', {
							url : '${pageContext.request.contextPath}/staff.do?add',
							success : function(data) {
								try {
									var d = $.parseJSON(data);
									if (d) {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/staff.do?datagrid',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
