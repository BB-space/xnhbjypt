...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${pageContext.request.contextPath}/invoicePicking/datagrid.do',
			toolbar : '#toolbar',
			title : '',
			iconCls : 'icon-save',
			pagination : true,
			...
			columns :[[{
				...
				width : 20,
				formatter:function(value,row,index){
					return '<button value="收款确定" onclick="picking(\''+row.id+'\')">拣货</button>';
				}
			}]]
		});
	...
	function picking(value){
		window.location.href="${pageContext.request.contextPath}/invoicePicking/invoicePickingDetail.do?id="+value;
	}
</script>
</head>
